...
DELIMITER // 
CREATE PROCEDURE procedure_to_register_user_with_administrator_role(
...
BEGIN 
    ...
    START TRANSACTION; 
		...
		VALUES (p_name, p_last_name, p_phone, p_address, p_email, p_password, 'ADMINISTRATOR', NOW(), NOW());
	COMMIT;
END //

# Procedure to register user with secretary role
DELIMITER // 
CREATE PROCEDURE procedure_to_register_user_with_secretary_role(
	IN p_name VARCHAR(50),
    IN p_last_name VARCHAR(50),
    IN p_phone VARCHAR(8),
    IN p_address VARCHAR(255),
...
BEGIN 
    ...
    BEGIN
		ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error registering user as secretary.';
	END;
    START TRANSACTION; 
		INSERT INTO user (name, last_name, phone, address, email, password, type_of_user, createdAt, updatedAt)
		VALUES (p_name, p_last_name, p_phone, p_address, p_email, p_password, 'SECRETARY', NOW(), NOW());
	COMMIT;
END //
